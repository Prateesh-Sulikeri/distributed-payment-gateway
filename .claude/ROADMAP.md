...
### 7e. Decided execution order within Phase 7 (decided 2026-07-04, supersedes the earlier coarse 8-step list)

Seven stages, dependency-ordered. Per-item detail stays in each service's `PENDING.md`; this is the sequence.

**Stage 1 — Critical security fixes** (small, independent, conflict-free — do first):
1. `payment-service`: make `ApiKeyValidationFilter` fail **closed** — 401 on missing/malformed `Authorization` for `/payments`, instead of today's pass-through → confusing 500.
2. `merchant-service`: add auth to its own management endpoints (register/rotate/update/get-by-id) — the biggest security hole in the system. Minimum viable: HTTP Basic with the already-present-but-unused `AUTH_ADMIN_*` env creds; leave `GET /validate-key` open for inter-service Feign calls (gateway as eventual single gate → `DECISIONS.md`).
3. `merchant-service`: enforce `MerchantStatus` in `validateApiKey` + a way to transition status.

**Stage 2 — Shared contracts library** (7b): extract a shared Gradle module for `MerchantResponse`, `MerchantStatus`, `PaymentMethod`, `FailureReason`, `TransactionStatus`, `CurrencyCode`, and the three Kafka event DTOs; migrate all six services. Do it before the bulk of per-service work so Stage 3 fixes are written against shared types once, not migrated afterward. (Mechanism — composite build vs. mavenLocal publish — decided at implementation time; there is no root Gradle build today.)

**Stage 3 — Per-service correctness fixes (7a), each landing with Testcontainers tests** (tests are part of each item, not a separate stage). Service order is dependency-driven; 3d/3e are independent and parallelizable across sessions:
- **3a. `merchant-service`** (contract provider — first): `@ControllerAdvice` → 404/400; `@Valid`; springdoc; **per-merchant webhook-secret field/endpoint** (prerequisite for 3c's HMAC signing); delete dead deps/`.env` vars.
- **3b. `payment-service`**: outbox publish idempotency; decide + implement `payment.succeeded` durability (outbox vs. documented at-least-once → `DECISIONS.md`); cache eviction from `PaymentProcessedConsumer`; stop `GlobalExceptionHandler` leaking raw messages; delete dead `jwt.*`/`auth.*`/`mock_bank.url` config; regenerate `.env.example`.
- **3c. `webhook-service`** (needs 3a's secret): consumer exception handling so genuine failures rethrow (making `@RetryableTopic` live) while expected conditions are recorded, not dropped; dedup on `payment_id`; distributed lock on `WebhookRetryJob`; wire or delete unused `@Retry`/`@TimeLimiter` YAML; HMAC-SHA256 `X-Webhook-Signature`; backoff-schedule decision → `DECISIONS.md`; fix `idx_webhook_retry` to cover `FAILED`; delete dead DTO/`.env` vars.
- **3d. `mock-bank-service`** (independent): wire `BankConfig` into the processors (roadmap's original configurability bar); consumer idempotency guard via the already-available `idempotencyKey`; `@RateLimiter` around the simulation; fix the `spring.application.name` indentation bug; delete dead code (`BankPaymentMethod`, `ACCOUNT_BLOCKED`); cheap pure-logic unit tests.
- **3e. `settlement-service`** (independent): build `GET /settlements` + manual trigger endpoint; catch the same-day-rerun `DataIntegrityViolationException` (merge-or-skip per merchant, don't fail the job); distributed lock on the scheduled job; assign real `SettlementStatus` values; chunk-vs-tasklet decision → `DECISIONS.md`; cron-time decision; springdoc once endpoints exist. Document (don't yet implement) whether payment-service ever learns `SETTLED`.
- **3f. `gateway-service`**: env-driven route targets (hard prerequisite for Stage 5); auth at the gateway (delegate to `/validate-key`); per-route circuit breakers; route PUT/PATCH/DELETE; WireMock-backed routing tests. Rate limiting stays in Stage 6.

**Stage 4 — Containerization groundwork** (7c steps 1-2) — ✅ **done 2026-07-04**: all Feign URLs (payment-service, webhook-service → merchant-service) and gateway-service's four route targets externalized to env vars; `scripts/provision_kafka_topics.py` + a `kafka-topic-init` compose service now provision all three business topics + `.DLT` variants (retry-index topics still self-provisioned by Spring Kafka, as designed); repo-wide `.env` cleanup done (dead vars removed per-service, standardized to `DB_HOST`/`DB_PORT`/`DB_NAME`/`DB_USER`/`DB_PASSWORD`/`SERVER_PORT`/`SPRING_PROFILES_ACTIVE` everywhere, every service now has a checked-in `.env.example`). See `CHANGELOG.md` and each service's `PENDING.md` (dated 2026-07-04 entry) for exact detail.

**Stage 5 — Containerize** (7c steps 3-4) — ✅ **done and end-to-end verified 2026-07-04**: multi-stage `Dockerfile` for all six services; `docker-compose.yml` now runs the full system (all six app services + infra, `depends_on` gated on real healthchecks, not just start order); dual Kafka listeners added for container networking. `docker compose up -d --build` was run to completion: all 13 containers reached `healthy`, and a real payment was driven through the gateway (`POST /payments` → 202 → async `SUCCESS`), confirming gateway→payment-service, payment-service→merchant-service (Feign), payment-service↔Kafka, mock-bank-service↔Kafka, webhook-service's fan-out consumption + Feign lookup + delivery-retry scheduling, and settlement-service's fan-out consumption all work correctly over container-to-container Docker networking, not just `localhost`. One real bug was found and fixed in this verification pass: `kafka-python==2.0.2` (used by `scripts/provision_kafka_topics.py`) is incompatible with Python 3.12 (`ModuleNotFoundError: kafka.vendor.six.moves`) — the `kafka-topic-init` image now pins `python:3.11-slim`.

Note: several **config-only** items originally scoped under Stage 3 above were pulled forward and completed alongside Stages 4-5 in the same 2026-07-04 pass, since they were pure infra/config cleanup with no business-logic risk: `ddl-auto` standardized to `validate` (payment-service, settlement-service), mock-bank-service's `spring.application.name` bug fix, `BankConfig` wired into the processors, dead dependency/config removal across the board, Actuator + graceful shutdown + dev/sit/prod profiles added everywhere, and webhook-service's `idx_webhook_retry` index fix. **The actual correctness/business-logic items in 3a-3f remain open** — no authentication was added anywhere (payment-service's fail-open filter and merchant-service's wide-open endpoints are untouched), no idempotency/dedup/HMAC/distributed-lock work was done, and Stage 1/Stage 2 haven't started. Re-check each service's `PENDING.md` for what's actually still outstanding versus what the original Stage 3 bullets above describe.

**Stage 6 — Observability & polish** (7d — deliberately last, per the reasoning below): Micrometer + Zipkin tracing (trace ID originates at the gateway); Prometheus + Grafana dashboard; Redis-backed gateway rate limiting (100 req/min per merchant); DLT visibility (`GET /admin/dlt-messages` or `@DltHandler`-based monitor); Swagger aggregation at the gateway; Bruno collection; architecture diagram in README.

**Continuous (not a stage)**: `DECISIONS.md` entries written the moment each decision is made (payment.succeeded durability, webhook backoff, chunk-vs-tasklet, cron time, SETTLED-status question, merchant-auth model) — not batched at the end; several discrepancies found in this review (settlement's chunk size, the webhook backoff schedule, the settlement cron time) exist precisely because a decision was made once and never revisited against what shipped. `PROGRESS.md` updated as stages land.

**Explicitly deferred to Phase 8+**: multiple API keys per merchant (and the key-format separator fix that rides along with it), merchant lifecycle Kafka events, non-blocking `simulateLatency`, the bulkhead pattern, analytics/aggregate endpoints, `SETTLED` feedback into payment-service (unless 3e's decision says otherwise).

**Why this order**: webhook HMAC needs a per-merchant secret → 3a before 3c. Gateway route externalization (3f) + topic provisioning (Stage 4) are hard prerequisites of containerization (Stage 5). Contracts library early so per-service fixes aren't written twice. Security fixes even earlier: tiny, critical, conflict-free. Observability last — tracing a request across services that aren't yet reliably network-reachable to each other (pre-containerization) is of limited value, and chasing dashboards before the underlying data model is sound just visualizes bugs.
